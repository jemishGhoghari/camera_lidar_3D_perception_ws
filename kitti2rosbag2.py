...
def save_imu_data(bag, kitti, imu_frame_id, topic):
    ...
    for timestamp, oxts in zip(kitti.timestamps, kitti.oxts):
        ...
        imu.header.stamp = stamp_to_ros_time(float(timestamp.strftime("%s.%f")))
        ...
        timestamp_ns = int(float(timestamp.strftime("%s.%f")) * 1e9)
...
def save_dynamic_tf(bag, kitti, kitti_type, initial_time):
    ...
    if kitti_type.find("raw") != -1:
        for timestamp, oxts in zip(kitti.timestamps, kitti.oxts):
            ...
            tf_oxts_transform.header.stamp = stamp_to_ros_time(
...
def save_camera_data(
    ...
    if kitti_type.find("raw") != -1:
        ...
        with open(os.path.join(image_dir, "timestamps.txt")) as f:
            image_datetimes = [
                datetime.strptime(x.strip(), "%Y-%m-%d %H:%M:%S.%f")
    ...
    for dt, filename in bar(iterable):
        ...
        if kitti_type.find("raw") != -1:
            timestamp_sec = float(datetime.strftime(dt, "%s.%f"))
            image_message.header.stamp = stamp_to_ros_time(timestamp_sec)
            topic_ext = "/image_raw"
        elif kitti_type.find("odom") != -1:
            timestamp_sec = dt
            image_message.header.stamp = stamp_to_ros_time(dt)
...
def process_single_drive(bag, args, date, drive, cameras, bridge):
...
def run_kitti2bag():
    ...
    parser.add_argument(
        ...
        "--multi-drive",
        nargs="+",
        help="multiple drives to merge into single bag (e.g., -m 0001 0002 0003). Must be used with --date for RAW datasets.",
    )
    parser.add_argument(
        "-o", "--output", help="output bag file name (without extension)"
    )
    ...
    if args.kitti_type.find("raw") != -1:
        if args.date == None:
            ...
            print(
                "For multiple drives: kitti2bag raw_synced [dir] -t <date> -m <drive1> <drive2> ..."
            )
            sys.exit(1)
        ...
        if args.multi_drive:
            ...
            if args.output:
                ...
                bag_name = "kitti_{}_drives_{}_{}".format(
                    args.date, "_".join(drives), args.kitti_type[4:]
                )
        elif args.drive:
            drives = [args.drive]
            if args.output:
                ...
                bag_name = "kitti_{}_drive_{}_{}".format(
                    args.date, args.drive, args.kitti_type[4:]
                )
        else:
            print(
                "Drive option is not given. Use -r for single drive or -m for multiple drives."
            )
            print(
                "Usage for raw dataset: kitti2bag raw_synced [dir] -t <date> -r <drive>"
            )
        ...
        for camera in cameras:
            ...
            topic_metadata_map[camera[2] + "/camera_info"] = (
        ...
        for topic, msg_type in topic_metadata_map.items():
            ...
            # Set QoS for tf_static to TRANSIENT_LOCAL
            if topic == "/tf_static":
                # Create offered_qos_profiles with TRANSIENT_LOCAL durability
...
- history: 3
  ...
  avoid_ros_namespace_conventions: false"""
        ...
        try:
            success_count = 0
            for drive in drives:
                if process_single_drive(bag, args, args.date, drive, cameras, bridge):
                    success_count += 1

            print("\n" + "=" * 60)
            print(
                "Successfully processed {} out of {} drives".format(
                    success_count, len(drives)
                )
            )
            print("=" * 60)
        ...
        if args.output:
            ...
            bag_name = "kitti_data_odometry_{}_sequence_{}".format(
        ...
        if args.sequence in odometry_sequences[:11]:
            print(
                "Odometry dataset sequence {} has ground truth information (poses).".format(
                    args.sequence
                )
            )
            kitti.load_poses()
