...
def process_single_drive(bag, args, date, drive, cameras, bridge, time_offset=0.0):
    ...
    save_dynamic_tf(
        bag, kitti, args.kitti_type, initial_time=None, time_offset=time_offset
    )
...
def run_kitti2bag():
    ...
    if args.kitti_type.find("raw") != -1:
        ...
        storage_options = rosbag2_py.StorageOptions(
            uri=bag_name, storage_id=args.format
        )
        ...
        try:
            ...
            for i, drive in enumerate(drives):
                result = process_single_drive(
                    bag, args, args.date, drive, cameras, bridge, time_offset
                )
                if result:
                        ...
                        if success:
                            ...
                            if i < len(drives) - 1:  # Not the last drive
                                first_timestamp_next = float(
                                    pykitti.raw(args.dir, args.date, drives[i + 1])
                                    .timestamps[0]
                                    .strftime("%s.%f")
                                )
                                # Calculate offset: last_timestamp + gap - first_timestamp_next
                                time_offset = (
                                    last_timestamp + 0.1 - first_timestamp_next
                                )
                    else:
                        if result:
                            success_count += 1
