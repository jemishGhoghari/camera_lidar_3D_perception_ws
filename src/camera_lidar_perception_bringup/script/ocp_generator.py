...
class RenderTopBoxesService(Node):
    def __init__(self):
        super().__init__("render_top_boxes_service")
        ...
        self.service_name = "/render_top_boxes"

        self.cb_srv = MutuallyExclusiveCallbackGroup()
        self.cb_cli = MutuallyExclusiveCallbackGroup()

        qos = QoSProfile(depth=50)
        self.sub = self.create_subscription(Detection3DArray, self.dets_topic, self._on_dets, qos)
        self.srv = self.create_service(Trigger, self.service_name, self._handle_render, callback_group=self.cb_srv)
        self.map_client = self.create_client(GetMap, self.map_service_name, callback_group=self.cb_cli)

        self.accum: Dict[str, List[Dict]] = {}
        self._lock = threading.Lock()

        self.get_logger().info(f"Subscribed to {self.dets_topic}; service {self.service_name} ready.")
    ...
    def _on_dets(self, msg: Detection3DArray):
        ...
        with self._lock:
            for det in msg.detections:
                if not det.results:
                    continue
                best = max(det.results, key=lambda r: getattr(r.hypothesis, "score", 0.0))
                ...
                yaw = math.atan2(2.0 * (bw * bz + bx * by), 1.0 - 2.0 * (by * by + bz * bz))
    ...
    def _handle_render(self, req, resp):
        ...
        for cid, c in chosen:
            world_poly = self._top_corners_xy(c["cx"], c["cy"], c["dx"], c["dy"], c["yaw"])
        ...
        for poly, cid in kept_info:
            ...
            cv2.putText(out, cid, (cx + 1, cy + 1), font, base_scale, (0, 0, 0), thickness + 2, cv2.LINE_AA)
            cv2.putText(out, cid, (cx, cy),         font, base_scale, color,           thickness,   cv2.LINE_AA)
        ...
        render_json = {
            ...
            "map": {
                "width": w, "height": h, "resolution": resm,
    ...
    def _grid_to_bgr(self, grid: OccupancyGrid) -> np.ndarray:
        ...
        gray = np.where(dat < 0, 127, np.clip(240 - (dat.astype(np.float32) * 2.0), 40, 240)).astype(np.uint8)
        gray = np.flipud(gray)
        return cv2.cvtColor(gray, cv2.COLOR_GRAY2BGR)

    def _top_corners_xy(self, cx: float, cy: float, dx: float, dy: float, yaw: float) -> np.ndarray:
        hx, hy = 0.5 * dx, 0.5 * dy
    ...
    def _world_poly_to_img_px(self, poly_xy: np.ndarray, ox: float, oy: float, oyaw: float, resm: float, img_h: int) -> np.ndarray:
        pts = []
