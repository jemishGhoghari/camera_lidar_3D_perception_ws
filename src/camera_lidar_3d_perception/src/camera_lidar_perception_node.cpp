  ...
  : Node("camera_lidar_3d_node", options)
{
  ...
  sync_->registerCallback(std::bind(&CameraLidarPerceptionNode::syncCallback, this,
...
void CameraLidarPerceptionNode::syncCallback(
    ...
    const sensor_msgs::msg::CameraInfo::ConstSharedPtr& camera_info_msg)
{
  ...
  projectCloud(point_cloud_cam, detections_msg, camera_info_msg, pointcloud_msg->header,
               detection3d_array_msg, detection_cloud_msg);

  detection_3d_pub_->publish(detection3d_array_msg);
}

void CameraLidarPerceptionNode::transformPointCloud(
    ...
    sensor_msgs::msg::PointCloud2& combine_detection_cloud_msg)
{
  ...
  size_t detections_nonempty = 0;
  size_t kept_points_total   = 0;
  size_t clusters_kept       = 0;
  ...
  for (size_t i = 0; i < yolo_detections_msg->detections.size(); ++i)
  {
    ...
    detections_nonempty++;
    kept_points_total += detection_cloud_raw->size();
    ...
    pcl::PointCloud<pcl::PointXYZ>::Ptr bbox_cloud =
        (clustered_detection_cloud && !clustered_detection_cloud->empty())
          ? clustered_detection_cloud
          : detection_cloud_in_target;

    if (bbox_cloud && !bbox_cloud->empty())
      clusters_kept++;

    *combine_detection_cloud += *bbox_cloud;

    createBoundingBox(detection3d_array_msg, bbox_cloud, yolo_detections_msg->detections[i].results);
  }
}

bool CameraLidarPerceptionNode::project3DToPixelRectified(
    const sensor_msgs::msg::CameraInfo::ConstSharedPtr& cam_info_msg,
    const cv::Point3d& p3d,
    cv::Point2d& uv)
{
...
void CameraLidarPerceptionNode::processPointsWithBbox(
    ...
    pcl::PointCloud<pcl::PointXYZ>::Ptr& raw_detection_cloud)
{
  ...
  for (const auto& pt : cloud_cam->points)
  {
    ...
    if (pt.z > 60.0) continue;              
    if (std::abs(pt.x) > 40.0) continue;    
    if (std::abs(pt.y) > 20.0) continue;
...
pcl::PointCloud<pcl::PointXYZ>::Ptr
CameraLidarPerceptionNode::cloud2TransformedCloud(
...
CameraLidarPerceptionNode::eucludianClustering(const pcl::PointCloud<pcl::PointXYZ>::Ptr& cloud)
{
  ...
  for (const auto& idx : cluster_indices)
  {
    ...
    for (int i : idx.indices) clu->push_back((*cloud)[i]);
...
CameraLidarPerceptionNode::createMarkerArray(
    const vision_msgs::msg::Detection3DArray& /*detection3d_array_msg*/,
